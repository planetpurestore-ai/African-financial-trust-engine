...
CMD ["sh", "-c", "alembic upgrade head && python -m app.bootstrap_runtime && exec uvicorn app.production_entry:app --host 0.0.0.0 --port ${PORT:-10000} --proxy-headers --forwarded-allow-ips='*' --no-server-header"]
