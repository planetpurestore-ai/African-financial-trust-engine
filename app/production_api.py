...
def create_organization(body: OrganizationCreate, x_bootstrap_token: str | None = Header(default=None, alias="X-Bootstrap-Token"), db: Session = Depends(db_session)):
    ...
    if not expected or not x_bootstrap_token or not secrets.compare_digest(x_bootstrap_token, expected): raise HTTPException(403, "Bootstrap authorization required")
    org = Organization(name=body.name.strip()); db.add(org); db.flush()
