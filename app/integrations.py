...
def google_vision_document_text(data: bytes) -> str:
    """Extract dense document text from an image using Google Cloud Vision.

    The provider is optional: the endpoint remains usable without OCR credentials,
    but image processing returns a clear configuration error until a provider is set.
    """
    url = "https://vision.googleapis.com/v1/images:annotate"
    payload = {
        "requests": [{
            "image": {"content": __import__("base64").b64encode(data).decode("ascii")},
...
def image_ocr(data: bytes, content_type: str) -> dict[str, Any]:
    ...
    text = google_vision_document_text(data)
    return {"provider": "google_vision", "content_type": content_type, "text": text}


def verify_webhook_signature(raw_body: bytes, signature: str | None, secret: str | None = None) -> bool:
    secret = secret or os.getenv("WEBHOOK_SIGNING_SECRET")
...
def normalize_financial_event(provider: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Normalize bank/mobile-money webhook payloads into a provider-neutral record."""
    def first(*keys):
        for key in keys:
